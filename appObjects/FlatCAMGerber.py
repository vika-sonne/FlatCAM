	...
	def plot(self, kind=None, **kwargs):
		...
		try:
			if self.options["solid"]:
				for g in geometry:
					if type(g) == Polygon or type(g) == LineString:
						self.add_shape(shape=g, color=color,
									   face_color=get_rand_fg_color(self.app) if self.options['multicolored']
				...
				for g in geometry:
					if type(g) == Polygon or type(g) == LineString:
						self.add_shape(shape=g, color=get_rand_fg_color(self.app) if self.options['multicolored'] else 'black',
									   visible=visible)
					elif type(g) == Point:
						pass
					else:
						for el in g:
							self.add_shape(shape=el, color=get_rand_fg_color(self.app) if self.options['multicolored'] else 'black',
										   visible=visible)
			self.shapes.redraw(
