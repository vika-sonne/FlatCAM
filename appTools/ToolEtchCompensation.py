		...
		if ratio_type == 'factor':
			try:
				...
				self.app.inform.emit(
					f'[ERROR_NOTCL] {_("Error while Etch Factor calculating")}: "{self.ui.factor_entry.text()}"')
			...
			offset = self.ui.offset_entry.get_value() / 1000   # from microns to mm

		self.app.inform_shell.emit(f'Tool Etch: {offset=} mm')
